 ...
 * trimmed to the fields this scaffold uses. */
typedef struct {
    ...
    uint8_t     channels;   /* # discrete channels (for convenience / labelling) */
    const char *name;       /* human-readable part number */
} p1_module_props_t;

/* The subset of the P1000 catalog this scaffold knows about. Every `id` and
 * `do`/`di` byte count is quoted from Module_List.h at the cited line; the
 * `ch`/name/description fields are for readability.
 * IDs verified against docs/references/p1am-library/Module_List.h (v1.0.10).
 * Output-type/channel descriptions: see docs/references/facts-docs/<part>.md. */
static const p1_module_props_t P1_MODULE_DB[] = {
    /*  id            di  do  ch  name            Module_List.h line + notes */
    { 0x1404008FUL,   0,  1,  8,  "P1-08TRS"  },  /* :66  8-pt relay (6x Form A + 2x Form C) */
    { 0x14040091UL,   0,  2, 16,  "P1-16TR"   },  /* :68  16-pt relay output */
    { 0x14050081UL,   0,  1,  8,  "P1-08TD1"  },  /* :72  8-pt sinking DC output */
    { 0x14050082UL,   0,  1,  8,  "P1-08TD2"  },  /* :74  8-pt sourcing DC output */
    { 0x04A00081UL,   1,  0,  8,  "P1-08ND3"  },  /* :46  8-pt DC input (for combo demos) */
    { 0x24A50081UL,   1,  1,  8,  "P1-16CDR"  },  /* :80  8 DC in + 8 relay out combo */
};

#define P1_MODULE_DB_COUNT (sizeof(P1_MODULE_DB) / sizeof(P1_MODULE_DB[0]))

/* Sentinel returned for an ID we don't recognise. */
static const p1_module_props_t P1_MODULE_UNKNOWN = { 0xFFFFFFFFUL, 0, 0, 0, "UNKNOWN" };
